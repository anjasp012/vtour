...
<div class="bg-white rounded-lg shadow-sm max-w-2xl">
    <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800">New Interactive Point</h3>
    </div>
        ...
        <form action="{{ route('admin.views.infospots.store', $view) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type-selector" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                    <option value="info" {{ old('type') == 'info' ? 'selected' : '' }}>Information Icon (Popup)</option>
                    <option value="nav" {{ old('type') == 'nav' ? 'selected' : '' }}>Navigation Arrow (Go to Scene)</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <div>
                    ...
                    <input type="number" name="position_x" value="{{ old('position_x', 5000) }}" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pos Y</label>
                    <input type="number" name="position_y" value="{{ old('position_y', 0) }}" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Pos Z</label>
                    <input type="number" name="position_z" value="{{ old('position_z', 0) }}" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm">
                </div>
            </div>
            ...
            <div id="info-fields">
                <div class="mb-4">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Content (ID)</label>
                    <textarea name="content_id" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('content_id') }}</textarea>
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Content (EN)</label>
                    <textarea name="content_en" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('content_en') }}</textarea>
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">3D Model (.glb file) - Optional</label>
                ...
                <div class="mb-4">
                    ...
                    <select name="target_scene_id" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        @foreach($hasTargetScenes as $ts)
                            <option value="{{ $ts->id }}" {{ old('target_scene_id') == $ts->id ? 'selected' : '' }}>{{ $ts->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
